...
define(["require", "exports", "src/editor/elements/HTMLElement"], function (require, exports, HTMLElement_1) {
    ...
    let HTMLEDropzoneInputElementBase = class HTMLEDropzoneInputElementBase extends HTMLElement {
        constructor() {
            super();
            (0, HTMLElement_1.bindShadowRoot)(this, /*html*/ `
            <style>
                :host {
                    display: block;
                }
        ...
        connectedCallback() {
            ...
            if (dropzoneSlot) {
                dropzoneSlot.addEventListener("slotchange", () => {
                    const dropzone = dropzoneSlot.assignedElements().filter(elem => (0, HTMLElement_1.isTagElement)("e-dropzone", elem));
            ...
            if (inputSlot) {
                inputSlot.addEventListener("slotchange", () => {
                    const input = inputSlot.assignedElements().filter(elem => (0, HTMLElement_1.isTagElement)("input", elem));
                    if (input.length > 0) {
                        this.input = input[0];
                    }
                });
