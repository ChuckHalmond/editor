...
function ReactiveNode(objectOrList, node, react) {
    if ("clear" in objectOrList) {
        const listener = ((event) => {
            react(node, event.detail.addedItems, event.detail.removedItems, event.detail.index);
        });
        Object.assign(node, {
        ...
        react(node, objectOrList, [], 0);
    }
    else {
        const listener = ((event) => {
            react(node, event.detail.property, event.detail.oldValue, event.detail.newValue);
        });
        Object.assign(node, {
            _reactiveNodeAttributes: {
                addReactListener: () => {
...
function ReactiveChildNodes(list, map, placeholder) {
    return (parent) => {
        const listener = ((event) => {
            const listItems = list.getAll();
            if (event.detail.addedItems.length === listItems.length) {
                parent.textContent = "";
            }
            ...
            if (event.detail.addedItems.length) {
                let addedElements = event.detail.addedItems.map(item => map(item));
                if (event.detail.index >= listItems.length - event.detail.addedItems.length) {
                    parent.append(...addedElements);
                }
                else {
                    parent.childNodes.item(event.detail.index - event.detail.removedItems.length).before(...addedElements);
                }
            }
            if (listItems.length === 0 && placeholder) {
        ...
        const listItems = list.getAll();
        const children = placeholder && listItems.length === 0 ? [placeholder] : listItems.map(map);
        return children;
    };
}
