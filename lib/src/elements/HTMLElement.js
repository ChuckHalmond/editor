...
;
function isReactiveChildNode(node) {
    return typeof node._reactiveChildIndex === "number";
}
function ReactiveChildNodes(list, map) {
    return (parent) => {
        const reactiveChildMap = (item, index) => {
            console.log(index);
        ...
        const listener = (event) => {
            ...
            if (event.data.addedItems.length) {
                if (event.data.index >= list.items.length - event.data.addedItems.length) {
                    ...
                    const addedElements = event.data.addedItems.map((item, index) => reactiveChildMap(item, list.items.length - event.data.addedItems.length + index));
                    if (lastChild) {
