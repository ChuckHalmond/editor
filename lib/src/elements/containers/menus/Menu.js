    ...
    connectedCallback() {
        ...
        if (slot) {
            slot.addEventListener("slotchange", () => {
                const items = slot.assignedElements().filter(elem => elem instanceof HTMLEMenuItemElement || elem instanceof HTMLEMenuItemGroupElement);
                this.items = items;
                items.forEach((item) => {
        ...
        this.addEventListener("mouseout", (event) => {
            const target = event.target;
            const thisIntersectsWithMouse = pointIntersectsWithDOMRect(event.clientX, event.clientY, this.getBoundingClientRect());
