...
class DropzoneDataBase {
    ...
    getData() {
        const dropzoneData = this._dropzone.multiple ? this._dropzone.draggables.map(draggable => draggable.data) :
            this._dropzone.draggables.length > 0 ? this._dropzone.draggables[0].data : null;
        const childDropzones = Array.from(this._dropzone.querySelectorAll("e-dropzone")).filter(dropzone => dropzone.parentElement.closest("e-dropzone") === this._dropzone);
